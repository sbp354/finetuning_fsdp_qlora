...
def get_dataloader(tokenizer: PreTrainedTokenizerFast, args: Dict):
    ...
    if args["dataset"] == "alpaca":
        ...
        dataset = load_dataset("sprice12345/{}".format(args["dataset"]), split = "train")
        dataset = dataset.shuffle(seed=args["seed"])
    ...
    if args["dataset"] == "guanaco":
        ...
        dataset = InstructionDataset(dataset, tokenizer, style=args["dataset"])
...
class LORA(nn.Module):
    def __init__(self, base_layer, lora_rank, lora_alpha, lora_dropout):
        ...
        lora_A = nn.Linear(
            base_layer.in_features, lora_rank, bias=False, device=device, dtype=dtype
        )
        lora_B = nn.Linear(
            lora_rank, base_layer.out_features, bias=False, device=device, dtype=dtype
        )
...
def fsdp_main(local_rank: int, world_size: int, args: Dict):
    ...
    print(
        "Model created", rank, f"{torch.cuda.memory_allocated(local_rank)/1e9:.3f} GB"
    )
    ...
    logger.log(
        {"memory_after_model_creation": torch.cuda.memory_allocated(local_rank)}, rank
    )
    ...
    print(
        "Wrapped model", rank, f"{torch.cuda.memory_allocated(local_rank)/1e9:.3f} GB"
    )
    logger.log(
        {"memory_after_model_wrap": torch.cuda.memory_allocated(local_rank)}, rank
    )
    ...
    for epoch in range(args["num_epochs"]):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if batch_idx == 4 and epoch == 0:
                logger.log(
                    {
                        "memory_before_forward": torch.cuda.memory_allocated(local_rank)
                        / 1e9
                    },
                    rank,
                )
            ...
            if batch_idx == 4 and epoch == 0:
                logger.log(
                    {
                        "memory_after_backward": torch.cuda.memory_allocated(local_rank)
                        / 1e9
                    },
                    rank,
                )
        ...
        if epoch == 0 and rank == 0:
            ...
            if args["verbose"]:
                print_func(
                    f"Peak memory usage (training): {peak_memory/1e9:.2f}GB", rank
                )
            if args["log_to"] == "wandb":
    ...
    if args["save_model"]:
        ...
        if args["train_type"] in ["custom_lora", "custom_qlora", "hqq_lora"]:
            cpu_state_dict = {}
            trainable_modules = [
                (n, m) for n, m in model.named_modules() if n.endswith("lora_AB")
            ]
...
def main(
    ...
    train_type: Param(
        "", choices=["full", "lora", "qlora", "custom_qlora", "custom_lora", "hqq_lora"]
    ...
    ) = "constant",  # Learning Rate Scheduler. linear and cosine warm up for 10% of training steps.
    log_to: Param(
    ...
    # Run
    mp.spawn(
        fsdp_main, args=(world_size, args), nprocs=torch.cuda.device_count(), join=True
    )
